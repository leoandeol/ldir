...
if __name__ == "__main__":
    ...
    for i, dataset in enumerate(datasets_train):
        ...
        # print(type(X))
        # eps = get_adaptive_radius_vat(X, train_domains[i], torch.cuda.current_device())
    ...
    for epoch in range(n_epoch):
        ...
        while i < len_dataloader:
            ...
            for j in range(n_iters):
                ...
                # embeddings, class_outputs, domain_outputs, mixed_y = net(
                #    x, seen_y, alpha
                # )
                ...
                # print(y_sup)
                # mx, amx  =torch.max(y_est_sup, 1)
                # mx = mx.detach().cpu().numpy()
                # amx = amx.detach().cpu().numpy()
                # print([f"{x}({y})" for x, y in zip(mx, amx)], "=?=", torch.argmax(y_sup, 1).detach().cpu().numpy())
                # print("CLASSIF LOSS[",j,"]=",classif_loss)
            ...
            # print("CLASSIF LOSS[FINAL]=",loss_classif)

            mean_domain_outputs_cond = torch.mean(torch.cat(all_y_mixed_semi)*torch.cat(all_domain_outputs), dim=0)
            mean_domain_outputs_normal = torch.mean(torch.cat(all_domain_outputs))
            loss_disc = 0
            for j in range(n_iters):
                if disc_type == "ours":
                    loss_disc_tmp = (all_y_mixed_semi[0] * all_domain_outputs[0]).mean(
                        dim=0
                    ) - mean_domain_outputs_cond
                    loss_disc += loss_disc_tmp.mean()
                elif disc_type == "joint" or disc_type == "marginal":
                    loss_disc += all_domain_outputs[j].mean() - mean_domain_outputs_normal
        ...
        net.train()
        errors = list([1-x for x in test_domains_scores.values()])
