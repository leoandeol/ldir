...
if __name__ == "__main__":
    ...
    dataset = get_dataset(domain, test=False, known=3000)
    
    dataset.epsilons = torch.ones((len(dataset.dataset.data))) * 2.5
    
    dataset_test = get_dataset(domain, test=True, known=-1)
    
    dataloader_train_semi = torch.utils.data.DataLoader(
        dataset, batch_size=batch_size, shuffle=True, drop_last=True
    )
    
    dataloader_train_sup = torch.utils.data.DataLoader(
    ...
    for epoch in range(n_epoch):
        data_iters_semi = iter(dataloader_train_semi)
        data_iters_sup = iter(dataloader_train_sup)
        i = 0

        len_dataloader = len(dataloader_train_semi)
        while i < len_dataloader:
            ...
            data_semi = data_iters_semi.next()
            data_sup = data_iters_sup.next()
        ...
        # }
        test_domains_scores = {domain: score_clf(net, dataloader_test)}
